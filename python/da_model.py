...
class encoder_RNN(nn.Module):
  ...
  def forward(self, x):
    ...
    for t in range(self.T - 1): 
      ...
      e = self.lin(e.view(-1, self.hidden * 2 + self.T - 1)) # attention pass
      a = ff.softmax(e.view(-1, self.features)) # ensure attention weights sum to 1
...
class decoder_RNN(nn.Module):
  ...
  def forward(self, x, y):
    ...
    for t in range(self.T - 1): 
      ...
      l = self.lin_in(l.view(-1, 2 * self.hidden + self.enc_hidden)) # attention pass
      b = ff.softmax(l.view(-1, self.T - 1)) # attention weights that sum 1
...
def train_da_model(encoder, decoder, train_x, train_y, T, features,
 enc_state_path = "encoder_state.pkl", dec_state_path = "decoder_state.pkl", epochs = 1000):
  """
  ...
  for epoch in range(epochs):
    ...
    while (idx < tsteps):
      ...
      tx_tensor = torch.from_numpy(tx).cuda()
      ty_tensor = torch.from_numpy(ty).cuda()
      target_y = torch.from_numpy(train_y[train_idxs + T])
      ...
      if (float(loss) < best_loss):
        msg = "\ntrain_loss = {:.3f} | epoch = {:.1f}\n".format(float(loss), float(epoch))
...
if (args.cleaned):
  ...
  enc_state_file = "enc_state_" + str(N) + "d_clean"
  dec_state_file = "dec_state_" + str(N) + "d_clean"
else:
  raw_file = "../input_files/input_vol" + str(N) + "d.csv"
  dec_state_file = "dec_state_" + str(N) + "d"
  enc_state_file = "enc_state_" + str(N) + "d"
